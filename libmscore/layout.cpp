...
void Score::createMMRest(Measure* firstMeasure, Measure* lastMeasure, const Fraction& len)
      ...
      //
      Segment* lastMeasureEndBarSeg = lastMeasure->findSegmentR(SegmentType::EndBarLine, lastMeasure->ticks());
      if (lastMeasureEndBarSeg) {
            Segment* mmrEndBarSeg = mmrMeasure->undoGetSegmentR(SegmentType::EndBarLine, mmrMeasure->ticks());
            for (int staffIdx = 0; staffIdx < nstaves(); ++staffIdx) {
                  Element* e = lastMeasureEndBarSeg->element(staffIdx * VOICES);
                  if (e) {
                        bool generated = e->generated();
                        if (!mmrEndBarSeg->element(staffIdx * VOICES)) {
                              Element* eClone = generated ? e->clone() : e->linkedClone();
                              eClone->setGenerated(generated);
                              eClone->setParent(mmrEndBarSeg);
                              undoAddElement(eClone);
                              }
                        else {
                              BarLine* mmrEndBarline = toBarLine(mmrEndBarSeg->element(staffIdx * VOICES));
